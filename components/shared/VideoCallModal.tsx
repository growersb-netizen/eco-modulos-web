...
export default function VideoCallModal({ isOpen, onClose, productoDefault }: VideoCallModalProps) {
  ...
  const waLink = `https://wa.me/5491126036495?text=${encodeURIComponent('Hola, quiero agendar una videollamada gratuita para evaluar ' + (PRODUCTOS.find((p) => p.value === form.producto)?.label || 'un producto') + '.')}`
